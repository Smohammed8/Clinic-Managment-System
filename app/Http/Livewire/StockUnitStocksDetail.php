...
class StockUnitStocksDetail extends Component
{
    use WithPagination;
    ...
    public function mount(StockUnit $stockUnit): void
    {
    ...
    {
        $this->editing = false;
        $this->modalTitle = trans('crud.stock_unit_stocks.new_title');
        $this->resetStockData();
    ...
    {
        $this->editing = true;
        $this->modalTitle = trans('crud.stock_unit_stocks.edit_title');
        $this->stock = $stock;
    ...
    public function save(): void
    {
        ...
        if (!$this->stock->stock_unit_id) {
            $this->authorize('create', Stock::class);

            $this->stock->stock_unit_id = $this->stockUnit->id;
        } else {
    ...
    public function toggleFullSelection(): void
    {
        ...
        foreach ($this->stockUnit->stocks as $stock) {
    ...
    {
        return view('livewire.stock-unit-stocks-detail', [
            'stocks' => $this->stockUnit->stocks()->paginate(20),
        ]);
    }
}
