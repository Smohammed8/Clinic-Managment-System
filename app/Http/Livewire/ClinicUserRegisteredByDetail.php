...
class ClinicUserRegisteredByDetail extends Component
{
    use WithPagination;
    ...
    protected $rules = [
        ...
        'encounter.doctor_id' => ['required', 'exists:clinic_users,id'],
    ];
    ...
    {
        $this->editing = false;
        $this->modalTitle = trans('crud.clinic_user_registered_by.new_title');
        $this->resetEncounterData();
    ...
    {
        $this->editing = true;
        $this->modalTitle = trans('crud.clinic_user_registered_by.edit_title');
    ...
    public function save()
    {
        ...
        if (!$this->encounter->registered_by) {
            $this->authorize('create', Encounter::class);

            $this->encounter->registered_by = $this->clinicUser->id;
        } else {
    ...
    {
        return view('livewire.clinic-user-registered-by-detail', [
