                    ...
                    <table class="table table-hover  table-sm table-condensed">
                        ...
                        <tbody>
                            @php
                                $i = 1;
                            @endphp
                            @forelse($pharmacyUsersWithPharmacy as $user)
                                <tr>
                                    <th>{{ $i++ }}</th>
                                    <td>{{ $user->name ?? '-' }}</td>
                                    <td>{{ $user->pharmacyUsers->pharmacy->name ?? 'Not Assigned' }}</td>
                                    {{-- <td>{{ $store->description ?? '-' }}</td>
                                    <td>{{ $store->status ?? '-' }}</td> --}}
                                    <td class="text-center" style="width: 134px;">
                                        <div role="group" aria-label="Row Actions" class="btn-group">
                                            <a href="{{ route('store_and_pharmacy_users.assignPharamacyPlace', $user) }}">
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="5">{!! $pharmacyUsersWithPharmacy->render() !!}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
